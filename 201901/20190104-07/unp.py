...
@Date: 2019-01-07 10:08:45
@LastEditTime: 2019-01-07 16:24:02
@Description: 
'''
...
def fnmatch(pattern, filename):
    ...
    pattern = ps.path.normcase(pattern)
...
class UnpackerBase(object):
    ...
    @classmethod
    def mimetypes_matches(cls, filename):
    ...
    def unpack(self, dst):
        ...
        try:
            if self.real_unpack(tmp_dir) != 0:
                click.secho('Error: unpacking through %s failed' % self.executable, fg='red')
                sys.exit(2)
...
def get_unpacker_class(filename):
    ...
    for unpacker_cls in unpackers:
        if unpacker_cls.mimetypes_matches(filename):
            return unpacker_cls
...
def cli(files, silent, output, dump_command, forced_unpacker):
    """
    np is a super simple command line application that can unpack a lot
