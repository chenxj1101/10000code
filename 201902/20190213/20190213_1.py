...
@Date: 2019-02-13 14:54:09
@LastEditTime: 2019-02-21 16:38:51
@Description: 公众号爬虫
'''
...
def pipe_articles(fakeid, query=''):
    ...
    print(f"本次搜索到: {total_page} 页文章, 已处理: {curr_searched} 页, 共在{len(data)} 条文章链接 ...")
...
def crawl_baidu_pan_link(url, sdir, url_cache):
    ...
    try:
        ...
        print(f"failed crawl links from url:{url}")
        sg = tracback.format_exc()
        print(sg)
        return False
...
def pipe_crawl_articles(arti_info):
    title_4_dir = arti_info['title'].replace(':', '').replace(' ', '').replace(':', '').replace(':', '').replace('/', '').replace('|', '').replace('<', '').replace('>', '').replace('?', '').replace('"', '')
    sdir = os.path.join(Input.out_dir, Input.fake_name, title_4_dir)
    if not os.path.exsits(sdir):
        os.makedirs(sdir, exist_ok=True)
